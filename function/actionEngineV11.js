class Validators {
    ...
    static isSingleRequest(obj) {
        console.log()
        return obj && operate.isObject(obj) && obj.objectModel && obj.method && !obj.flowRequest && !obj.element;
    }
    ...
    static isForLoopRequest(obj){
        return obj && operate.isObject(obj) && obj.objectModel && obj.element && obj.forLoop && operate.isArray(obj.forLoop);
    }
...
//This can easily be replaced by operate.isin(key,object)
    async processReq(reqObj,resultObj) {
        ...
        if(Validators.isForLoopRequest(reqObj)){
            return await this.processForLoop(reqObj, resultObj);
        }
     ...
     */
    async processReqArray(reqObj,state={}) {
        if(operate.isFlowRequest(reqObj)&&operate.isArray(reqObj.flowRequest)) {
            ...
            for(var i=0;i<flowRequest.length;i++) {
               ...
               // var updatedRequest = {...request,arguments:requestArgs};
                var result= await this.processReq(request,state);
                console.log(result);
                //  if(result) {
                    state={
                        ...state,
    ...
    async processForLoop(req,state){
        var condition;
        if(req.hasOwnProperty('method')){
            condition = await this.processReq(req,state);
        }else{
            condition =await this.handleRequiredPreviousResults(state,req);
        }
        console.log("Condition :- " + condition);
        for await(var element of condition){
            console.log("In the for loop");
            state[req['element']] = element;
            for await(var reqObj of req.forLoop.length){
                console.log(reqObj);
                //var result =await this.processReq(request.forLoop[i],state);
                //if(request.forLoop[i].hasOwnProperty('reqName'))
                //    state[req.forLoop[i].reqName] = result;
            }
        }
    }
